		...
		eventCellOnClick: function (e, obj) {
			...
			if (daydata.enabled && daydata._month==0)
			{
				...
				if( !this.options.isFrontEnd ) {
					toggleSelectedDayAvailability(daydata.day);
				}
		...
		render:function() {
			...
			for (var k=1;k<7;k++)
			{
				...
				while (element)
				{
					...
					var dayContent = this.hideBoundaryDatesContent && dataobj._month!=0 ? "" : this.evaluateMarkup(this.options.dayListMarkup, dataobj );
					var contentHtml = "<div class='rich-calendar-cell-date-div'>" + dayContent + "</div>";
					...
					{
						contentHtml += "<div class='rich-calendar-cell-price-div'>";
						if( this.calendarDayInfo != null && this.calendarDayInfo[ dataobj.day - 1 ] != null ) {
							dataobj.dayAvailability = this.calendarDayInfo[ dataobj.day - 1 ].availability;
							if( this.options.isShowingPrices ) {
								if( ((dataobj.dayAvailability == 1 && !this.options.isFrontEnd) || dataobj.dayAvailability == 0) ) {
									contentHtml += "&pound;" + this.calendarDayInfo[ dataobj.day - 1 ].price + " " + this.calendarDayInfo[ dataobj.day - 1 ].rateUnit;
								} else {
						...
						contentHtml += "</div>";
						if( !this.options.isShowingPrices && dataobj.dayAvailability == 2 ) {
							dataobj.dayAvailability = 0;
						}
...
//						} 
//						else 
						if (!this.options.disabled && !this.options.readonly && dataobj.enabled) classNames+=' rich-calendar-btn';
						...
						if( dataobj.dayAvailability == 1 ) {
							classNames+=' rich-calendar-booked';
						} else if( dataobj.dayAvailability == 2 ) {
							classNames+=' rich-calendar-no-rate';
						}
		...
		changeCurrentDate: function(year, month, noUpdate) {
			if (this.getCurrentMonth()!=month || this.getCurrentYear()!=year)
			{
				...
				if (this.invokeEvent("currentdateselect", rf.getDomElement(this.id), null, date))
				{
					...
					currentDateStr = this.currentDate.getFullYear() + "-" + (this.currentDate.getMonth() + 1) + "-" + (this.currentDate.getDay() + 1);
		...
		calendarDatesLoaded: function( calendarDayInfo ) {
			this.calendarDayInfo = calendarDayInfo;
			this.onUpdate();
			this.invokeEvent("currentdateselected", $(this.id), null, this.currentDate);
