...
CodePress = {
	...
	snippets : function(evt) {
		...
		/*
		 * Modified to insert tab on FF @author Asad Khan <asadkn
		 * [at-the-rate-of] gmail [dot] com>
		 */
		if (noInsertion == 1) {
			...
			if (range.commonAncestorContainer.getElementsByTagName && false) {
				var childNodes = range.commonAncestorContainer
				...
				for ( var i = 0; i < childNodes.length; i++) {
					...
					// in range?
					if (srcRange.compareBoundaryPoints(Range.START_TO_START,
							range) >= 0
							&& srcRange.compareBoundaryPoints(Range.END_TO_END,
									range) <= 0) {
